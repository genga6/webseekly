...
class SearchNode(Node):
    def __init__(
        ...
        if not self.api_key or not self.search_engine_id:
            raise EnvironmentError("API_KEY or SEARCH_ENGINE_ID isnot set in the environment.")
    ...
    async def _search_multiple_queries(self, queries: list, num_results: int) -> list[dict]:
        ...
        Returns:
            dict: a flat list of search results.
        """
    ...
    async def _search_google(self, client: httpx.AsyncClient, query: str, num_results: int) -> list[dict]:
        ...
        if not results:
            raise ValueError("No results found from Google Custom Search API.")
    ...
    def execute(self, state) -> dict:
        ...
        """
        try:
            # Retrieve the current event loop or create a new one
